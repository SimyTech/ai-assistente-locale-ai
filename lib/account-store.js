...
async function persistStoredOwnerAccount(account, env = process.env) {
  const serialized = JSON.stringify(account);
  const email = normalizeLogin(account.email);
  const username = normalizeLogin(account.username);
  if (!email || !clean(account.id)) throw new Error("Account memorizzato non valido.");
...
export async function resetStoredOwnerPassword({ login, newPassword, tenantId }, env = process.env) {
  ...
  await persistStoredOwnerAccount(updated, env);
