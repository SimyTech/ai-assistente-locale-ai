...
export async function markStoredOwnerEmailVerified({ login, tenantId, accountId = "" }, env = process.env) {
  const account = await getStoredOwnerAccount(login, env);
  ...
  if (expectedAccountId && clean(account.id) !== expectedAccountId) {
    return { verified: false, reason: "account-mismatch" };
  }
...
export async function createStoredOwnerAccount({
  ...
  const account = {
    ...
    emailVerified: false,
    emailVerifiedAt: "",
    termsAcceptedAt: clean(termsAcceptedAt),
