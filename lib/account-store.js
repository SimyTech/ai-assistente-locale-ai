...
export async function changeStoredOwnerPassword({ login, currentPassword, newPassword, tenantId }, env = process.env) {
  ...
  const email = normalizeLogin(updated.email);
  const username = normalizeLogin(updated.username);
  if (!email || !clean(updated.id)) throw new Error("Account memorizzato non valido.");
  ...
  await redisCommand(env, "SET", accountIdKey(updated.id), serialized);
