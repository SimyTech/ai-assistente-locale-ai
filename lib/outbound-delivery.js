...
export function whatsappProactiveEnabled(env = process.env) {
  return clean(env.MAVIRI_WHATSAPP_PROACTIVE).toLowerCase() === "true";
}

export function whatsappDeliveryConfigured(tenantId = "default", env = process.env) {
  return Boolean(
    whatsappProactiveEnabled(env) &&
    clean(env.WHATSAPP_ACCESS_TOKEN) &&
    whatsappPhoneNumberIdForTenant(tenantId, env)
  );
}
...
export async function sendWhatsAppText({ to, text, tenantId = "default", authorizedManual = false }, env = process.env, fetchImpl = globalThis.fetch) {
  if (!whatsappProactiveEnabled(env) && authorizedManual !== true) {
    return { sent: false, reason: "whatsapp-proactive-disabled" };
  }
...
export async function deliverAuthorizedProposal(proposal = {}, tenantId = "default", env = process.env, fetchImpl = globalThis.fetch) {
  ...
  if (channel === "whatsapp") {
    const authorizedManual = proposal.approved === true && clean(proposal.sourceType) === "manual-whatsapp";
    return sendWhatsAppText({ to: proposal.recipient, text: proposal.text, tenantId, authorizedManual }, env, fetchImpl);
  }
  if (channel === "email") {
