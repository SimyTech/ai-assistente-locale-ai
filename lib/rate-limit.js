...
export const CLIENT_RATE_LIMITS = Object.freeze({
  ...
  chat: { limit: 30, tenantLimit: 180, globalLimit: 220, windowSeconds: 60 },
  availability: { limit: 40, tenantLimit: 240, globalLimit: 220, windowSeconds: 60 },
  ...
  "public-context": { limit: 60, tenantLimit: 300, globalLimit: 220, windowSeconds: 60 },
  context: { limit: 60, tenantLimit: 300, globalLimit: 220, windowSeconds: 60 }
});
...
export function globalRateLimitKey({ action }) {
  const value = String(action || "unknown").toLowerCase();
  const bucket = ["chat", "availability", "public-context", "context"].includes(value)
    ? "public-read"
    : ["book", "update", "cancel"].includes(value)
      ? "public-write"
      : value;
  return `maviri:global:rate:${bucket}`;
}
