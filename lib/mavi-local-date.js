...
function relativeNumericPeriod(text, now) {
  ...
  const weeksAgo = text.match(/\b(\d{1,2})\s+settiman[ae]\s+fa\b/);
  if (weeksAgo) {
    const count = Math.min(52, Math.max(1, Number(weeksAgo[1])));
    return dayRange(addDays(now, -7 * count), `${count} ${count === 1 ? "settimana" : "settimane"} fa`);
  }

  const weeksAhead = text.match(/\b(?:tra|fra)\s+(\d{1,2})\s+settiman[ae]\b/);
  if (weeksAhead) {
    const count = Math.min(52, Math.max(1, Number(weeksAhead[1])));
    return dayRange(addDays(now, 7 * count), `tra ${count} ${count === 1 ? "settimana" : "settimane"}`);
  }
  ...
  const yearsAgo = text.match(/\b(\d{1,2})\s+ann[oi]\s+fa\b/);
  if (yearsAgo) {
    const count = Math.min(20, Math.max(1, Number(yearsAgo[1])));
    return yearRange(now.getFullYear() - count, `${count} ${count === 1 ? "anno" : "anni"} fa`);
  }

  const yearsAhead = text.match(/\b(?:tra|fra)\s+(\d{1,2})\s+ann[oi]\b/);
  if (yearsAhead) {
    const count = Math.min(20, Math.max(1, Number(yearsAhead[1])));
    return yearRange(now.getFullYear() + count, `tra ${count} ${count === 1 ? "anno" : "anni"}`);
  }
  ...
  if (lastMonths) {
    ...
    return {
      ...
      label: `ultimi ${count} ${count === 1 ? "mese" : "mesi"}`,
      kind: "range"
    };
  }
