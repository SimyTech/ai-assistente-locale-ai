...
async function processTenant(dataKey, now) {
  ...
  for (const reminder of plan.due) {
    try {
      const delivery = await sendWhatsApp(reminder.recipient, reminder.message);
      if (delivery.sent) {
...
export default async function reminderHandler(req, res) {
  ...
  try {
    ...
    return json(res, 200, {
      ...
      whatsappConfigured: Boolean(clean(process.env.WHATSAPP_ACCESS_TOKEN) && clean(process.env.WHATSAPP_PHONE_NUMBER_ID)),
      results: tenants
    });
  } catch (error) {
