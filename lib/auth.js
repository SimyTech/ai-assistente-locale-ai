...
export function clientOwnsAppointment(appointment, body = {}) {
  const suppliedRaw = clean(body.phone || body.whatsapp || body.clientPhone || body.clientWhatsapp);
  const expectedRaw = clean(appointment?.phone || appointment?.whatsapp);
  const suppliedDigits = suppliedRaw.replace(/\D/g, "");
  const expectedDigits = expectedRaw.replace(/\D/g, "");
  if (suppliedDigits && expectedDigits && suppliedDigits === expectedDigits) return true;
  const supplied = phoneOwnershipIdentity(suppliedRaw);
  const expected = phoneOwnershipIdentity(expectedRaw);
