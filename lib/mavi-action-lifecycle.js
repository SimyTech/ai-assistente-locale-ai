...
export function createActionLifecycle() {
  ...
  function recordOutcome(proposal = {}, outcome = {}, now = Date.now()) {
    const action = ensure(proposal);
    if (action.status !== "completed") return { accepted: false, reason: "action-not-completed", action: { ...action } };
    const type = clean(outcome?.type);
    if (!OUTCOMES.has(type)) return { accepted: false, reason: "invalid-outcome", action: { ...action } };
  ...
  function list() {
    return [...actions.values()].map(action => ({ ...action, proposal: { ...action.proposal } }));
  }

  return { propose, edit, approve, requestSend, complete, recordOutcome, fail, retry, get, list, clear() { actions.clear(); } };
}
