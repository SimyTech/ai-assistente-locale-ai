...
export function normalizeTenantId(value, fallback = LEGACY_TENANT_ID) {
  ...
  return isValidTenantId(normalized) ? normalized : fallback;
}

export function explicitTenantId(req = {}, body = {}) {
  return req?.headers?.["x-maviri-tenant"] || body?.tenantId || body?.tenant || "";
}
...
export function tenantActionHistoryKey(tenantId) {
  const id = normalizeTenantId(tenantId, "");
  if (!id) return "";
  return id === LEGACY_TENANT_ID
    ? "maviri:action-history"
    : `maviri:tenant:${id}:action-history`;
}
