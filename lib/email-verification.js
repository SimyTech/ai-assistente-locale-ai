...
export async function requestEmailVerification(email, env = process.env) {
...
export async function consumeEmailVerification(token, env = process.env) {
  ...
  if (!email || !tenantId) return { verified: false, reason: "invalid-or-expired" };

  return markStoredOwnerEmailVerified({ login: email, tenantId }, env);
}
