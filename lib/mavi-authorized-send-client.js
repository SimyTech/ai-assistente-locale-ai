...
export async function requestAuthorizedSend(proposal = {}, options = {}) {
  ...
  try {
    // Usa l'endpoint esplicito: evita che una regola di rewrite CDN inoltri
    // l'azione autorizzata direttamente al Business Engine legacy.
    const response = await fetchImpl("/api/chat-entry", {
      method: "POST",
