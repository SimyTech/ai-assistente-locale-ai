...
function appointmentAnswer(message, data, now, forced = false) {
  ...
  const scheduleWords = /(programm|agenda|appuntament|impegn|giornata|recap|riepilog|chi (?:vedo|viene|ho)|come sono messo|che si fa|che facciamo|cosa c e)/.test(text);
  if (!period || !readIntent(text) || (!forced && !scheduleWords)) return null;
  const items = (data.appointments || [])
    .filter(item => item.date >= period.start && item.date <= period.end && (period.end < isoLocal(now) || activeAppointment(item)))
    .sort((a, b) => `${a.date || ""}${a.time || ""}`.localeCompare(`${b.date || ""}${b.time || ""}`));
  if (!items.length) return `Non risultano appuntamenti per ${period.label}.`;
...
function businessAnswer(message, data, forcedIntent = "") {
  ...
  if (forcedIntent === "address" || /(dove (?:siamo|siete)|qual e l indirizzo|indirizzo dell attivita)/.test(text)) {
    return business.address ? `L'indirizzo configurato è: ${business.address}.` : "Non risulta un indirizzo configurato.";
  }
  if (forcedIntent === "contact" || /(numero di telefono|telefono dell attivita|contatto dell attivita|numero whatsapp)/.test(text)) {
...
export function answerFastLocalData(message, data = {}, now = new Date()) {
  ...
  const semantic = detected.intent === "agenda" ? appointmentAnswer(message, data, now, true)
    ...
    : ["address", "contact"].includes(detected.intent) ? businessAnswer(message, data, detected.intent)
    : null;
