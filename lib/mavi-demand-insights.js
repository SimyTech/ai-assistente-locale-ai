...
function appointmentTime(item) {
  ...
  return clean(item?.start || item?.startsAt).match(/T(\d{2}:\d{2})/)?.[1] || "";
}
...
function servicePrice(item, services) {
  const direct = Number(item?.price ?? item?.amount);
  if (Number.isFinite(direct) && direct >= 0) return direct;
  const wanted = norm(item?.service || item?.serviceName);
  const service = services.find(candidate => norm(candidate?.name) === wanted);
...
function activeBands(body = {}) {
  ...
  if (!ranges.length) return BANDS;
  return BANDS.filter(band => {
    const bandMinutes = band.end - band.start;
    const maxOverlap = ranges.reduce((best, [start, end]) => {
      const overlap = Math.max(0, Math.min(end, band.end) - Math.max(start, band.start));
      return Math.max(best, overlap);
    }, 0);
    return maxOverlap >= bandMinutes * 0.5;
  });
}
...
export function findWeakTimeBands(body = {}, options = {}) {
  ...
  return rows
    .filter(row => row.appointments <= average * weaknessRatio)
    .map(row => ({
      type: "weak-time-band",
      priority: row.appointments === 0 ? "high" : "medium",
      action: "improve-demand",
      band: row.id,
      label: row.label,
      appointments: row.appointments,
      value: row.value,
      averageAppointments: average,
      lookbackDays,
      share: totalCompleted ? row.appointments / totalCompleted : 0,
      requiresApproval: true,
      autoExecute: false
    }))
    .sort((a, b) => a.appointments - b.appointments || a.value - b.value);
}
...
export function buildDemandInsight(body = {}, options = {}) {
  ...
  return {
    ...
    weakest,
    text: weakest
      ? `La fascia più debole è ${weakest.label}: ${weakest.appointments} prestazioni completate negli ultimi ${weakest.lookbackDays} giorni. Può essere utile concentrare qui una promozione o un richiamo mirato, sempre dopo conferma del titolare.`
      : "Non ci sono ancora dati sufficienti per individuare una fascia oraria debole in modo affidabile."
  };
}
