...
export function isConfirmation(text) {
  return /^(?:si|sì|ok|okay|confermo|conferma|va bene|perfetto|procedi|prenota)(?:\s|$|[,.!?])/i.test(clean(text));
}

export function isCancellation(text) {
  return /(?:^|\s)(?:no|annulla|annullare|cancella|cancellare|stop|lascia perdere)(?:\s|$|[,.!?])/i.test(clean(text));
}
