...
export function buildOperationalChatResponse(body = {}, options = {}) {
  ...
  const lines = [
    ...
    `• Valore cancellazioni recuperabili: €${Number(summary.recoverableValue || 0).toFixed(2)}`,
    `• Valore potenziale dei buchi agenda: €${Number(summary.agendaPotentialValue || 0).toFixed(2)}`,
    `• Opportunità economica complessiva: €${Number(summary.totalValueOpportunity || 0).toFixed(2)}`
  ];
