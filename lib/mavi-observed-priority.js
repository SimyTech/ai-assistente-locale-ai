...
export function rankProposalsByObservedOutcomes(proposals = [], actions = [], { minimumObserved = 3 } = {}) {
  ...
  return (Array.isArray(proposals) ? proposals : []).map((proposal, index) => {
    ...
    // Preferisce l'evidenza più specifica disponibile. Una sorgente ampia condivisa da più
    // proposte non deve sovrascrivere il risultato osservato della strategia o del servizio.
