...
function whatsappDraftsForService(service, band, data = {}) {
  return clientsForService(service, data).map(client => {
    const prepared = prepareOwnerApprovedCustomerMessage({
      kind: "smart_recall",
      client,
      context: { service, preferredBand: band }
    });
    return {
      kind: "message-draft",
      channel: prepared.channel,
      recipientName: prepared.clientName,
      recipient: prepared.phone,
      text: prepared.message,
      messageMode: prepared.messageMode,
      approved: prepared.approved,
      requiresApproval: prepared.requiresOwnerApproval,
      executable: false,
...
function weakBandDraft(item, data) {
  ...
  return {
    ...
    suggestedAudience, audienceRequiresApproval: true, whatsappDrafts, whatsappRequiresApproval: true
  };
}
