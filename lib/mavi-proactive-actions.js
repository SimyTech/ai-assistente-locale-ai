...
function clientsForService(service, data = {}) {
  ...
  for (const item of Array.isArray(data?.appointments) ? data.appointments : []) {
    ...
    const client = clients.find(row => clean(row?.id) && clean(row.id) === clean(item?.clientId))
...
function whatsappDraftsForService(service, band, data = {}) {
  const business = clean(data?.business?.name || data?.settings?.name) || "l'attività";
  return clientsForService(service, data)
    .map(client => ({
      kind: "message-draft",
      channel: "whatsapp",
      recipientName: clean(client?.name),
      recipient: clean(client?.whatsapp || client?.phone),
      text: `Ciao ${clean(client?.name)}, sono ${business}. Se ti fa comodo, in questo periodo stiamo cercando di valorizzare di più la fascia ${band}${service ? ` anche per ${service}` : ""}. Se vuoi, posso indicarti le disponibilità più comode.`,
...
function answerFor(proposal) {
  ...
  if (proposal.kind === "content-draft") {
    const strategy = proposal.sourceType === "weak-time-band" && proposal.recommendedService
      ? `\n\nStrategia suggerita: ${proposal.recommendedService}${proposal.suggestedAudience?.length ? `; pubblico iniziale: ${proposal.suggestedAudience.join(", ")}` : ""}${proposal.whatsappDrafts?.length ? `; bozze WhatsApp pronte: ${proposal.whatsappDrafts.length}` : ""}.`
      : "";
