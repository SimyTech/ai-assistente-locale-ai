...
const ACTION_WORDS = /(ricontatt|contatt|messaggio|scriv|preparam|recuper|riattiv|promozion|promo|buco|slot)/;
...
function matchBriefItem(message, brief) {
  ...
  if (/promozion|promo/.test(text)) return items.find(item => item.type === "promotion-expiry") || null;
...
export function createMaviProactiveActions() {
  ...
  function handle(message, brief, data = {}, conversationId = "default") {
    ...
    if (!item) {
      return {
        handled: true,
        answer: "A quale segnalazione ti riferisci? Indicami il cliente, la promozione o il buco in agenda.",
        proposal: null,
