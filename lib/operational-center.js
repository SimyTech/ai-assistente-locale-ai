...
function servicesForGap(services, minutes) {
  return services
    .map(service => ({
      id: clean(service?.id),
      name: clean(service?.name),
      duration: serviceDuration(service),
      price: serviceValue(service)
    }))
    .filter(service => service.name && service.duration && service.duration <= minutes)
    .sort((a, b) => b.price - a.price || b.duration - a.duration || a.name.localeCompare(b.name));
}

function enrichGap(gap, services) {
  const compatibleServices = servicesForGap(services, gap.minutes);
  const best = compatibleServices[0] || null;
  return {
    ...gap,
    compatibleServices,
    recommendedService: best,
    potentialValue: best?.price || 0
  };
}
...
function recentlyContacted(client, today, days = 30) {
  ...
  const cutoff = Date.parse(`${addDays(today, -days)}T00:00:00Z`);
  ...
  for (let offset = 0; offset < horizonDays; offset += 1) {
    ...
    for (const [start, end] of busy) {
      if (start > cursor && start - cursor >= minGapMinutes) {
        gaps.push(enrichGap({ type: "agenda-gap", date, start: minutesToTime(cursor), end: minutesToTime(start), minutes: start - cursor, priority: offset <= 1 ? "high" : "medium" }, services));
      }
      cursor = Math.max(cursor, end);
    }
    if (close > cursor && close - cursor >= minGapMinutes) {
      gaps.push(enrichGap({ type: "agenda-gap", date, start: minutesToTime(cursor), end: minutesToTime(close), minutes: close - cursor, priority: offset <= 1 ? "high" : "medium" }, services));
    }
  }
...
export function findInactiveClients(body = {}, options = {}) {
  ...
  return [...lastVisits.entries()].flatMap(([key, row]) => {
    ...
    const client = clientsById.get(clean(row.item?.clientId)) || clients.find(item => norm(item?.name) === norm(clientName(row.item, clientsById)));
...
export function findCancellationRecovery(body = {}, options = {}) {
  ...
  return appointments.flatMap(item => {
    ...
    const client = clients.find(candidate => clean(candidate?.id) === clean(item?.clientId)) || clients.find(candidate => norm(candidate?.name) === norm(item?.name || item?.clientName || item?.customerName));
    if (recentlyContacted(client, today)) return [];
...
export function buildOperationalCenter(body = {}, options = {}) {
  ...
  const actions = [
    ...
    ...gaps.map(item => ({ ...item, action: "fill-gap", label: item.recommendedService ? `Riempi il buco ${item.date} ${item.start}–${item.end} con ${item.recommendedService.name}` : `Riempi il buco ${item.date} ${item.start}–${item.end}` }))
  ];
  const priorityRank = { high: 0, medium: 1, low: 2 };
  actions.sort((a, b) => (priorityRank[a.priority] ?? 9) - (priorityRank[b.priority] ?? 9) || Number(b.value || b.potentialValue || 0) - Number(a.value || a.potentialValue || 0));
  const cancellationValue = cancellations.reduce((sum, item) => sum + item.value, 0);
  const agendaPotentialValue = gaps.reduce((sum, item) => sum + Number(item.potentialValue || 0), 0);
  return {
    ...
    summary: {
      ...
      recoverableValue: cancellationValue,
      agendaPotentialValue,
      totalValueOpportunity: cancellationValue + agendaPotentialValue
    },
    actions
  };
}
