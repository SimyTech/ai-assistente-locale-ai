...
export function resolveMaviOperationalContext(history = [], message = "", localData = {}) {
  ...
  const resolved = { date: "", time: "", service: "", client: "" };
  ...
  return { ...resolved, enrichedMessage: parts.filter(Boolean).join(" ").trim() };
}
...
function lastSubstantiveUserMessage(history = []) {
  const messages = (Array.isArray(history) ? history : [])
    .filter(item => item?.role === "user")
    .map(item => text(item.content, 1200))
    .filter(Boolean)
    .reverse();
  return messages.find(value => !/^(grazie|ok|va bene|perfetto|bene|si|sì|no)$/i.test(value)) || "";
}

export function resolveMaviManagerialFollowUp(history = [], message = "") {
  const current = text(message, 1200);
  const source = normalize(current);
  if (!source || source.length > 140) return { used: false, enrichedMessage: current, previous: "" };

  const isFollowUp = /^(e\b|invece\b|rispetto\b|solo\b|soltanto\b|quelli\b|quelle\b|nel\b|nella\b|ma\b|pero\b)/.test(source)
    || /\b(mese scorso|settimana scorsa|anno scorso|pomeriggio|mattina|sera|cancellat|annullat|completat|confermat|no show|assent)\b/.test(source);
  if (!isFollowUp) return { used: false, enrichedMessage: current, previous: "" };

  const previous = lastSubstantiveUserMessage(history);
  if (!previous || normalize(previous) === source) return { used: false, enrichedMessage: current, previous: "" };

  return {
    used: true,
    previous,
    enrichedMessage: `${previous} ${current}`.trim()
  };
}
