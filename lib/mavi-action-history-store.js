...
export async function saveActionHistorySnapshot(tenantId, snapshot, env = process.env) {
  const key = keyForTenant(tenantId);
  if (!snapshot || typeof snapshot !== "object" || Array.isArray(snapshot)) {
    throw new Error("Snapshot azioni non valido.");
  }
  await redisCommand(env, "SET", key, JSON.stringify(snapshot));
  return { saved: true, key };
}

export async function loadActionHistorySnapshot(tenantId, env = process.env) {
  const key = keyForTenant(tenantId);
...
export async function persistActionLifecycle(tenantId, lifecycle, env = process.env) {
  if (!lifecycle || typeof lifecycle.snapshot !== "function") {
    throw new Error("Lifecycle azioni non persistibile.");
  }
  return saveActionHistorySnapshot(tenantId, lifecycle.snapshot(), env);
}

export async function hydrateActionLifecycle(tenantId, lifecycle, env = process.env) {
  if (!lifecycle || typeof lifecycle.restore !== "function") {
    throw new Error("Lifecycle azioni non ripristinabile.");
  }
  const stored = await loadActionHistorySnapshot(tenantId, env);
...
export async function deleteActionHistory(tenantId, env = process.env) {
  const key = keyForTenant(tenantId);
