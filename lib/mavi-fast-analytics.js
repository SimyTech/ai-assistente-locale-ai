...
export function answerFastAnalytics(message, data = {}, now = new Date()) {
  ...
  if (!/(incass|fatturat|ricav|entrate|valore (?:generato|completato)|quanto ho fatto)/.test(text)) return null;
  const period = parseLocalAgendaPeriod(text, now);
  if (!period) return null;

  const current = completedValue(data, period);
  const wantsComparison = /(rispetto|confront|paragon|differenza|mese scorso|settimana scorsa|periodo precedente)/.test(text);
  if (!wantsComparison) {
  ...
  return `Valore stimato delle prestazioni completate per ${period.label}: ${euro(current)}. Periodo precedente comparabile: ${euro(before)}. Andamento ${direction}: ${variation}. È una stima gestionale basata sugli appuntamenti completati e sui prezzi configurati; non sostituisce la contabilità fiscale.`;
}
