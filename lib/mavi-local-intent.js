...
const VOCABULARY = Object.freeze({
  ...
  analytics: ["incasso", "incassi", "incassato", "fatturato", "fatturo", "ricavo", "ricavi", "entrate", "guadagno", "guadagni"],
...
const TIME = new Set(["oggi", "domani", "dopodomani", "ieri", "lunedi", "martedi", "mercoledi", "giovedi", "venerdi", "sabato", "domenica", "settimana", "settimane", "mese", "mesi", "anno", "anni", "gennaio", "febbraio", "marzo", "aprile", "maggio", "giugno", "luglio", "agosto", "settembre", "ottobre", "novembre", "dicembre"]);
...
export function analyzeLocalIntent(message) {
  ...
  const hasTime = tokens.some(token => TIME.has(token)) || /\b\d{1,2}(?:[/. -]\d{1,2})?\b/.test(text) || /\b(?:tra|fra)\s+\d{1,3}\s+(?:giorn[io]|settimane?|mes[ei]|anni?)\b/.test(text) || /\b\d{1,3}\s+(?:giorn[io]|settimane?|mes[ei]|anni?)\s+fa\b/.test(text);
  ...
  if (/(quanto (?:ho )?(?:incassato|fatturato|ricavato)|come stanno andando (?:gli )?incassi|confronta.*(?:incassi|fatturato|ricavi))/.test(text)) scores.analytics += 4;
