...
export const SESSION_COOKIE = "maviri_session_v2";
export const SESSION_COOKIE_DOMAIN = ".maviri.it";
...
export function sessionTenantId(token) {
  const [payload, suppliedSignature, extra] = String(token || "").split(".");
  if (!payload || !suppliedSignature || extra) return "";

  try {
    const data = JSON.parse(decode(payload));
    return normalizeTenantId(data?.tenantId, "");
  } catch {
    return "";
  }
}
...
export function sessionCookie(token) {
  return `${SESSION_COOKIE}=${encodeURIComponent(token)}; Path=/; Domain=${SESSION_COOKIE_DOMAIN}; Max-Age=${SESSION_TTL_SECONDS}; HttpOnly; Secure; SameSite=Strict`;
}

export function clearSessionCookie() {
  return `${SESSION_COOKIE}=; Path=/; Domain=${SESSION_COOKIE_DOMAIN}; Max-Age=0; HttpOnly; Secure; SameSite=Strict`;
}
